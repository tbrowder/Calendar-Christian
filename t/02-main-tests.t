...
#
# Copyright (c) 2021 Tom Browder <tbrowder@acm.org>
...
my $data; # defined in BEGIN block at eof
for $data.lines -> $line is copy {
    ...
    #note "DEBUG: '{$dt.raku}'"; #die "DEBUG exit";
    #next; # DEBUG

    is $dt.color, $color, "$date color";
...
my $dtccl = Date::Liturgical::Christian.new($dt);
is $dtccl.name, 'Pentecost', 'Can convert from DateTime';

$dtccl = Date::Liturgical::Christian.new(
    day=>4,
    month=>6,
    year=>2006);
...
is $dt.utc, '2006-06-03T00:00:00', 'Can convert to DateTime';
